...
// 多人Elo算法
void MultiElo(UserSet &us,ActionSet &as,int fromId,int round)
try
{
	//double fromIdElo=us.getEloValue(fromId);
	unsigned int n=as.m_Data.size();
	if(n>50){
		printf("range out\n");
		exit(-1);
	}
	...
	int i;
	for(iter=as.m_Data.begin(),i=0;iter!=as.m_Data.end();iter++,i++){
		ptoIdElo[i]=us.getEloValue(iter->second.gettoId());
		pActions[i]=iter->second.getAction();
	}
	...
	for(iter=as.m_Data.begin(),i=0;iter!=as.m_Data.end();iter++,i++){
		...
		us.setEloValue(iter->second.gettoId(),newElo);
	}
}
catch(exception &e){
	cerr<<e.what()<<endl;

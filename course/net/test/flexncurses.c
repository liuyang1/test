...
int scroll_menu(WINDOW **items,int cnt,int menu_start_col){
	int key;
	int selected=0;
	while(1){
...
int main(){
	...
	printw("Press F1 F2 to open menus");
	printw("ESC quit");
	refresh();
	do{
		int selected_item;
		...
		if(key==KEY_F(1)){
			...
			if(selected_item<0)
				...
				wprintw(messagebar,"You have selecte menu item %d",selected_item+1);
			touchwin(stdscr);
			refresh();
		}
		else if(key==KEY_F(2)){
